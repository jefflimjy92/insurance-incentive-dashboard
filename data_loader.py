...
def classify_product(row: pd.Series) -> str:
    ...
    # 4. 자동차보험 (New Position: 인보험/재물보다 우선)
    # "자동차보험은 계약종류 장기야" -> 장기 계약인 운전자/자동차 상품도 여기서 포착
    if 계약종류 == '자동차' or \
       any(kw in 상품명 for kw in ['자동차', '운전자', '이륜차', '비사업', '사업']) or \
       상품종류 in ['이륜차', '비사업', '사업', '자동차']:
        return '자동차보험'

    # 5. 재물
    # 조건: 상품종류=재물성 (단, 상품명 "펫", "자동차" 관련은 이미 위에서 걸러짐)
    if 상품종류 == '재물성':
        return '재물보험'

    # 6. 인보험
    # 조건: 상품종류=보장성 AND 계약종류=장기
    # (단, "실손, 펫, 단체, 자동차" 제외 -> 위에서 이미 걸러졌으므로 나머지 조건만 확인)
