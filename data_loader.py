...
def classify_product(row: pd.Series) -> str:
    ...
    # 2. 자동차보험 (계약종류 '자동차' 우선 + 키워드)
    # 이륜차, 비사업(개인용), 사업(영업용) 등은 자동차보험으로 분류
    if 계약종류 == '자동차' or 상품종류 in ['이륜차', '비사업', '사업'] or '자동차' in 상품명:
        return '자동차보험'
    ...
    # 5. 인보험
    # 사용자의 필수 규정: 상품종류가 '보장성'이어야 함 (종신보험 포함)
    # 플러스: 실손/펫 제외
    if 상품종류 in ['보장성', '종신'] or 계약종류 == '생보':
        if '실손' not in 상품명 and '펫' not in 상품명:
            return '인보험'
        else:
            return '기타'
    
    # 6. 기타 키워드 기반 보완 (명확한 경우만)
    if any(kw in 상품명 for kw in ['건강', '암', '상해', '질병']):
        if '실손' not in 상품명:
            return '인보험'
    if any(kw in 상품명 for kw in ['화재', '재산', '배상']) and '펫' not in 상품명:
        return '재물보험'
