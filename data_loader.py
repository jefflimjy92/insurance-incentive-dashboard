...
def load_rules_from_url(spreadsheet_url: str, sheet_name: str = "시상규칙") -> pd.DataFrame:
    ...
    # [CRITICAL] 3번째 컬럼(C열)을 강제로 '상품구분'으로 지정 (사용자 요청)
    if len(df.columns) >= 3:
        c_col = df.columns[2]
        df = df.rename(columns={c_col: '상품구분'})
...
def classify_product(row: pd.Series) -> str:
    """
    상품 분류 로직 (사용자 정의 규칙 v2)
    1. 실손: 상품명에 "실손"
    2. 펫보험: 상품명에 "펫"
    3. 단체: (상품종류=보장성 & 계약종류=장기 & 상품명="단체") OR (상품종류=단체)
    4. 재물: 상품종류=재물성 (단, "펫" 제외)
    5. 인보험: 상품종류=보장성 & 계약종류=장기 (단, "실손, 펫, 단체" 제외)
    """
    상품명 = str(row.get('상품명', '')).strip()
    상품종류 = str(row.get('상품종류', '')).strip()
    계약종류 = str(row.get('계약종류', '')).strip()
    계약자 = str(row.get('계약자', '')).strip()
    모집인명 = str(row.get('모집인명', '')).strip()
    사원명 = str(row.get('사원명', '')).strip()

    # 0. 본인계약 (최우선 확인 - 전처리에서 걸러지지만 안전장치)
    current_agent = 모집인명 if 모집인명 else 사원명
    if current_agent and 계약자 == current_agent:
        return '본인계약'
    ...
    # 2. 실손/운전자: 인보험 중 주요 분류 (자동차보험 체크 전 수행)
    if '실손' in 상품명:
        return '실손보험'
    if '운전자' in 상품명:
        return '인보험'
    ...
    # 3. 단체
    # 조건 A: 상품종류=보장성 AND 계약종류=장기 AND 상품명="단체"
    # 조건 B: 상품종류=단체
    if (상품종류 == '보장성' and 계약종류 == '장기' and '단체' in 상품명) or \
       (상품종류 == '단체'):
        return '단체보험'

    # 4. 자동차보험
    # 사용자의 요청에 따라 오직 '계약종류'가 '자동차' 포인트인 경우만 분류
    if 계약종류 == '자동차':
        return '자동차보험'
    ...
    # 6. 인보험
    # 조건: 상품종류=보장성 AND 계약종류=장기 (나머지 장기 보장성)
    if 상품종류 == '보장성' and 계약종류 == '장기':
        return '인보험'
...
def preprocess_contracts(df: pd.DataFrame, agent_name: Optional[str] = None) -> Tuple[pd.DataFrame, dict]:
    ...
    # [Fix] 중복 컬럼 제거 (ValueError: Operands are not aligned 방지)
    # 중복된 컬럼명 존재 시 result['컬럼'] 호출이 DataFrame을 반환하여 비교 연산 오류 발생 가능
    ...
    if '모집인명' in result.columns:
        ...
        # 모집인명이 원본에 아예 없으면 사원명을 모집인명으로 사용
        result['모집인명'] = result['사원명'].astype(str).str.strip()
        
    # 기존 코드 호환성을 위해 설계사 컬럼도 유지하되 모집인명 참조
    if '모집인명' in result.columns:
    ...
    # 본인 계약 제외 (모집인명 == 계약자 또는 사원명 == 계약자)
    result = result.reset_index(drop=True)  # [Fix] 인덱스 정렬 문제 방지
    before = len(result)
...
def filter_by_products(contracts: pd.DataFrame, 포함상품: Optional[str], 상품구분: Optional[str] = None) -> pd.DataFrame:
    ...
    if not pd.isna(상품구분) and str(상품구분).strip() != '':
        ...
        # '분류' 컬럼이 있으면 우선 사용 (preprocess_contracts에서 이미 분류됨)
        if '분류' in contracts.columns:
            ...
            # 2. 정확한 분류 매칭
            # (데이터의 분류값: 인보험, 재물보험, 단체보험, 펫보험, 자동차보험, 실손보험 등)
            return contracts[contracts['분류'] == category]
            
        # '분류' 컬럼이 없는 경우(비상시) - 기존 키워드 매칭 로직 유지
        # 카테고리별 키워드 매핑
        category_keywords = {
            '인보험': ['인', '생명', '상해', '질병', '암', '건강'],
            '펫보험': ['펫', '애완', '반려'],
...
def get_unique_agents(df: pd.DataFrame) -> List[str]:
    """
    계약 데이터에서 고유한 모집인명 목록 추출
    """
    if '모집인명' not in df.columns:
        return []
    return sorted(df['모집인명'].dropna().unique().tolist())
