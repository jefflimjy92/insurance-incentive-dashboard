...
def calculate_single_award(contracts: pd.DataFrame, rule_group: pd.DataFrame,
    ...
    if pd.notna(rule_start) and award_type != '연속형':
        calc_start = max(pd.Timestamp(period_start), pd.to_datetime(rule_start))
        
    calc_end = period_end
    if pd.notna(rule_end) and award_type != '연속형':
        calc_end = min(pd.Timestamp(period_end), pd.to_datetime(rule_end))
    
    if calc_start > calc_end and award_type != '연속형':
        return None
    else:
        # 3. 유형별 계산
        if award_type == '정률형':
            print(f"DEBUG: calc_rate_type input rows: {len(filtered_contracts)}")
            ...
            # Fallback: if consecutive rules mismatch (e.g. date) or empty return, try step-based logic if columns exist
            # Check if 0 amount AND column-based steps exist in rule
            ...
            if (not result or result.get('지급금액', 0) == 0) and has_step_columns:
                 # print(f"DEBUG: Consecutive rule fallback to Step Type for {rule_group['시상명'].iloc[0]}")
                 fallback_result = calc_step_type(filter_by_period(filtered_contracts, calc_start, calc_end), rule_group)
                 
                 # Only use fallback if it actually gives a better result (money > 0)
                 # Or if original result was None/Empty
                 if isinstance(fallback_result, dict) and fallback_result.get('지급금액', 0) > 0:
                     # Preserve period_stats from the original consecutive calculation if available
